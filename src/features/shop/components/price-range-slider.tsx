...
export function PriceRangeSlider({ priceRange }: PriceRangeSliderProps) {
  const searchParams = useSearchParams();
  const [isPending, startTransition] = useTransition();
  const maxPriceValue = Number(priceRange?.max_price ?? 0) / 100;
  const [maxPrice, setMaxPrice] = useState<number>(
    searchParams.get('maxPrice') ? Number(searchParams.get('maxPrice')) : maxPriceValue
  );
      ...
      <Accordion.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
        <div className="pt-3">
          ...
          <div className="mt-3 flex items-center justify-between">
            <span className="text-sm text-black">{priceRange?.currency_symbol}0</span>
