...
export async function SingleAttribute({ attribute }: { attribute: { id: number; name: string; taxonomy: string } }) {
  const attributeDataResponse = await apiFetchWithoutAuth(`/wc/store/v1/products/attributes/${attribute.id}/terms`);

  if (!attributeDataResponse.ok) {
    return null;
  }

  const attributeTerms = (await attributeDataResponse.json()) as { id: number; name: string; slug: string }[];

  return (
    <Accordion.Item value={attribute.id.toString()} className="py-5">
      <Accordion.Header>
        <Accordion.Trigger className="group flex w-full cursor-pointer items-center justify-between">
          <h3 className="font-satoshi-bold text-xl text-black">{attribute.name}</h3>
          <svg
      ...
      <Accordion.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
        <div className="pt-3">
          <SingleTerm attributeTerms={attributeTerms} attribute={attribute} />
        </div>
      </Accordion.Content>
    </Accordion.Item>
  );
